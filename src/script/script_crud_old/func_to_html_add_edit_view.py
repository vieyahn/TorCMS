...
def gen_input_add(sig):
    html_fangjia = '''
    <label for="{0}"><span><a class="glyphicon glyphicon-star" style="color: red;font-size: xx-small;"></a> {1}</span></label>

    <input id='{0}' name="{0}" value="" type="text"  class="input_text"> {2}
...
def gen_input_edit(sig):
    ...
    <input id='{0}' name="{0}" value="{{{{ post_info.infor['{0}'][0] }}}}" type="text"  class="input_text"> {2}
    '''.format(sig['en'], sig['zh'], sig['dic'][1])
    return (edit_fangjia)
...
def gen_input_view(sig):
    ...
    <div class="col-sm-8"><span class="val">{{{{ post_info.infor['{0}'][0] }}}} {2}</span></div>
    '''.format(sig['en'], sig['zh'], sig['dic'][1])
    return (out_str)
...
def gen_select_add(sig):
    ...
    <select id="{0}" name="{0}" class="input_text">
    '''.format(sig['en'], sig['zh'])
...
def gen_select_view(sig):
    ...
    for key in dic_tmp.keys():
        tmp_str = '''
         <span>
          {{% set tmp_var = post_info.infor["{0}"][0] %}}
          {{% if tmp_var == "{1}" %}}
          {2}
          {{% end %}}
         </span>
